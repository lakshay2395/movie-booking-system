...
class ShowsController < HallsController
  ...
  before_action -> { fetch_show }, only: %i[show destroy update]
  before_action -> { update_show_details_before_save }, only: [:update]

  SHOW_NOT_FOUND = 'No show found for id %s'
  ...
  def show
    show_model(@show, format(SHOW_NOT_FOUND, id), :not_found)
  end
  ...
  def destroy
    destroy_model(@show, format(SHOW_NOT_FOUND, id), :method_not_allowed)
  end

  def update
    update_model(@show, format(SHOW_NOT_FOUND, id), :method_not_allowed)
  end
